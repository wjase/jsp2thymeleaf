 ...
 * @author jason
 */
public class JSP2ThymeleafStreamConverter implements JspTreeConverterContext
{

    public static final Logger logger = Logger.getLogger(JSP2ThymeleafStreamConverter.class.getName());
    ...
    public JSP2ThymeleafStreamConverter()
    {
        scanForConverters();
    ...
    public void convert(InputStream inputStream, OutputStream outputStream)
    {
        try
        {
            final JspTree jspTree = Optional.ofNullable(parse(new String(IOUtils.toByteArray(inputStream))))
                    .orElseThrow(() -> new RuntimeException("Failed to parse JSP file"));
            logger.log(Level.FINE, jspTree.toStringTree());
            writeTree(jspTree, outputStream);
        } catch (IOException ex)
        {
            Logger.getLogger(JSP2ThymeleafStreamConverter.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private List<Content> rootContentFor(JspTree jspTree)
    {
        ...
        final Optional<Content> foundHtmlElement = contents.stream().filter(JSP2ThymeleafStreamConverter::isHtmlElement).findFirst();
...
//                break;
            ...
            return contents;
        } catch (JspNodeException je)
        {
            throw je;
        } catch (Throwable t)
        {
            throw new JspNodeException(t, jspTree);
        }
        ...
        switch (type)
        {
            ...
            case JSP_DIRECTIVE:
                return jspDirectives.converterFor(jspTree)
                        .orElseThrow(() -> new JspNodeException("No jsp directive converter found", jspTree));
